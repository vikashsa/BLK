...
class Blockchain {
  ...
  replace_chain(chain) {
    console.log(`the chain of nodes is ${chain}`);
    let network = blockchain.nodes;
    let longest_chain;
    let max_length = chain.length;
    console.log(max_length);
    console.log(network);

    //  var arr = network.toArray();
    var arr = Array.from(network);
    for (var i = 0; i < arr.length; i++) {
      //  console.log(arr[i].name+" iterated");

      //  for (let node of network) {
      //network.forEach(()=>{})
      console.log("iterating inside for loop");
      let response = request(
        ...
        (error, response, body) => {
          abcde = "fghkl";
          console.log(abcde);
          console.log(`the first response code is ${response.statusCode}`);
        }
      );
      //  console.log(`the global response code is ${global.globalresponseCode}`);
...
app.listen("5002", () => {
  console.log("Server is up on port 5002");
});
